...
with get_db_conn() as conn:
    ...
    if row:
        body_log_id, default_weight = row
        default_weight = float(default_weight)
        mode = "update"
        st.success(f"📌 {selected_date}에 기록된 체중: **{default_weight:.1f} kg**")
        ...
        if row:
            default_weight = float(row[0])
        else:
            # 3️⃣ 시작 체중
            cur.execute(
                """
                SELECT weight_kg_baseline
                FROM user_profile
                WHERE user_id = :u
                """,
                {"u": user_id_raw},
        ...
        st.info(f"ℹ️ {selected_date}에는 아직 체중 기록이 없습니다.")
